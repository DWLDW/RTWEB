...
- browser verification is still pending for the latest `/schedule`, `/attendance`, `/payments`, `/library`, and `/exams` UI changes

Quick verification done:
- `C:\Users\tooya\AppData\Local\Python\bin\python.exe -m py_compile app.py`

Next recommended task:
- run one browser pass on `/schedule`, `/attendance`, `/payments`, `/library`, and `/exams`, then continue the remaining query-mode cleanup where full lists still auto-open
