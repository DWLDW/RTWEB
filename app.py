...
I18N_TEXTS["en"].update({
    ...
    "attendance.makeup_date": "Makeup Date",
    "attendance.makeup_done": "Completed Makeup"
})
I18N_TEXTS["ko"].update({
    ...
    "attendance.makeup_date": "보강 날짜",
    "attendance.makeup_done": "완료된 보강"
})
I18N_TEXTS["zh"].update({
...
def render_sort_th(label, sort_key, current_sort_by, current_sort_dir, base_path, lang, keep_params, page_param="page"):
    cleaned_params = {k: v for k, v in (keep_params or {}).items() if k not in ("page", page_param)}
...
def complete_makeup_assignment(conn, student_user_id, schedule_id, attendance_id):
    ...
    source_row = conn.execute(
        "SELECT id, student_id, class_id, credit_delta FROM attendance WHERE id=?",
        (assignment["source_attendance_id"],),
    ).fetchone()
    source_credit_unit = get_class_credit_unit(conn, source_row["class_id"]) if source_row else 1.0
    source_new_delta = -float(source_credit_unit)
    if source_row:
        apply_credit_adjustment(conn, source_row["student_id"], source_row["credit_delta"], source_new_delta)
    conn.execute(
        """UPDATE attendance
        SET absence_charge_type='deduct', requires_makeup=1, makeup_completed=1, makeup_attendance_id=?, credit_delta=?
        WHERE id=?""",
        (attendance_id, source_new_delta, assignment["source_attendance_id"]),
    )
...
def app(environ, start_response):
    ...
    if path == "/users":
        ...
        if method == "POST" and has_role(user, [ROLE_OWNER, ROLE_MANAGER]):
            ...
            if action == "delete_user":
                delete_user_id = (data.get("user_id") or "").strip()
                delete_row = conn.execute("SELECT id, role FROM users WHERE id=?", (delete_user_id,)).fetchone() if delete_user_id.isdigit() else None
                dependency_checks = [
                    ("students", "user_id"),
                    ("teachers", "user_id"),
                    ("sessions", "user_id"),
                    ("classes", "teacher_id"),
                    ("classes", "foreign_teacher_id"),
                    ("classes", "chinese_teacher_id"),
                    ...
                    ("homework_submissions", "feedback_teacher_id"),
                    ("homework_submissions", "feedback_by"),
                    ("exam_scores", "student_id"),
                    ("counseling", "student_id"),
                    ("counseling", "parent_id"),
                    ("counseling", "created_by"),
                    ("payments", "student_id"),
                    ("announcements", "created_by"),
                    ("notifications", "target_user_id"),
                    ("book_loans", "student_id"),
                    ("book_loans", "handled_by"),
                ]
                if not delete_row:
                    add_error(errs, "user_id", "삭제할 사용자를 찾을 수 없습니다")
                elif str(delete_row["id"]) == str(user["id"]):
                    add_error(errs, "user_id", t("users.delete_self"))
                elif delete_row["role"] == ROLE_OWNER:
                    add_error(errs, "user_id", t("users.delete_owner"))
                else:
                    for table_name, field_name in dependency_checks:
                        if conn.execute(f"SELECT 1 FROM {table_name} WHERE {field_name}=? LIMIT 1", (delete_user_id,)).fetchone():
                            add_error(errs, "user_id", t("users.delete_blocked"))
                            break
                if errs:
                    ...
                    try:
                        conn.execute("BEGIN")
                        conn.execute("DELETE FROM teachers WHERE user_id=?", (delete_user_id,))
                        conn.execute("DELETE FROM students WHERE user_id=?", (delete_user_id,))
                        conn.execute("DELETE FROM sessions WHERE user_id=?", (delete_user_id,))
                        conn.execute("DELETE FROM users WHERE id=?", (delete_user_id,))
                        conn.commit()
                        flash_msg = t("users.deleted")
                        edit_user_id = (query.get("edit_user_id", "") or "").strip()
                        if edit_user_id == delete_user_id:
                            query["edit_user_id"] = ""
                    except Exception:
                        conn.rollback()
                        flash_msg = "사용자 삭제 실패"
                        flash_type = "error"
                        log_event(conn, "ERROR", path, "사용자 삭제 실패", traceback.format_exc(), user["id"])
                        conn.commit()
    ...
    if path.startswith("/classes/"):
        ...
        class_makeup_rows = conn.execute(
                """SELECT
                    ...
                    cmp.lesson_date AS makeup_lesson_date,
                    tgt.day_of_week,
                    tgt.start_time,
                    tgt.end_time,
                ...
                LEFT JOIN users ht ON ht.id=st.homeroom_teacher_id
                LEFT JOIN attendance cmp ON cmp.id=ma.completed_attendance_id
                WHERE tgt.class_id=?
        ...
        if export in ("students_csv", "attendance_csv"):
            if export == "students_csv":
                ...
                lines = ["lesson_date,student_name,status,absence_charge_type,requires_makeup,makeup_completed,makeup_attendance_id,makeup_date,note"]
        ...
        class_makeup_html = rows_html(class_makeup_rows, ["student_no", "student_name", "homeroom_teacher_name", "source_lesson_date", "source_class_name", "makeup_lesson_date", "day_of_week", "start_time", "end_time", "assignment_status"])
        ...
        <table>
          <tr><th>{t('students.field.student_no')}</th><th>{t('field.student')}</th><th>{t('students.field.homeroom_teacher')}</th><th>{t('field.date')}</th><th>{t('academics.class_name')}</th><th>{t('attendance.makeup_date')}</th><th>{t('academics.day_of_week')}</th><th>{t('academics.start_time')}</th><th>{t('academics.end_time')}</th><th>{t('academics.status')}</th></tr>
          {class_makeup_html}
        </table>
        </div>
        <div class='card'>
    ...
    if path == "/masterdata":
        ...
        <div class='card'><div class='filter-row'>
          ...
          <a class='btn {'secondary' if md_view!='packages' else ''}' href='/masterdata?lang={CURRENT_LANG}&md_view=packages'>{t('masterdata.packages')}</a>
        </div></div>
        """
        ...
        package_card = f"<div class='card'><h4>{t('masterdata.packages')}</h4>{helper_html}<form method='post' class='form-row'><input type='hidden' name='type' value='payment_package'><label>{t('masterdata.package_code')} <input name='code' placeholder='RT30'></label><label>{t('masterdata.package_name')} <input name='name' placeholder='RT30 30 credits'></label><label>{t('masterdata.package_credits')} <input name='lesson_credits' type='number' step='0.5' placeholder='30'></label><label>{t('masterdata.package_list_price')} <input name='list_price' type='number' step='0.01' placeholder='9900'></label><label>{t('academics.status')} <select name='status'><option value='active'>{status_t('active')}</option><option value='inactive'>{t('status.ended')}</option></select></label><button>{t('common.add')}</button></form><div class='table-wrap'><table><tr><th>{t('masterdata.package_code')}</th><th>{t('masterdata.package_name')}</th><th>{t('masterdata.package_credits')}</th><th>{t('masterdata.package_list_price')}</th><th>{t('academics.status')}</th><th>{t('common.delete')}</th></tr>{package_rows_md if load_master else ''}{(f"<tr><td colspan='6' class='empty-msg'>{t('common.no_data')}</td></tr>") if (load_master and not package_rows_md) else ''}</table></div></div>"
        section_body = classes_card if md_view == 'classes' else course_cards if md_view == 'courses' else room_cards if md_view == 'rooms' else package_card
        if md_view == "classes":
            register_card = f"""
    ...
    if path == "/schedule":
        ...
        if schedule_ids:
            assignment_rows = conn.execute(
                ...
                WHERE ma.target_schedule_id IN ({','.join(['?'] * len(schedule_ids))})
                  AND (
                    COALESCE(ma.status,'assigned')='assigned'
                    OR (COALESCE(ma.status,'assigned')='completed' AND COALESCE(cmp.lesson_date,'')=?)
                  )
                ORDER BY ma.id DESC""",
                tuple(schedule_ids) + (selected_view_date,),
            ).fetchall()
                    ...
                    if makeup_items:
                        makeup_names = ", ".join([
                            (
                                ...
                                if (item["status"] or "assigned") == "assigned"
                                else f"{item['name_ko'] or '-'} ({item['makeup_lesson_date'] or selected_view_date})"
                            )
                            for item in makeup_items[:3]
                        ])
                        more_suffix = "" if len(makeup_items) <= 3 else f" +{len(makeup_items)-3}"
                        pending_count = len([item for item in makeup_items if (item["status"] or "assigned") == "assigned"])
                        completed_count = len(makeup_items) - pending_count
                        label_prefix = t('attendance.makeup_pending') if pending_count and not completed_count else (
                            t('attendance.makeup_done') if completed_count and not pending_count else f"{t('attendance.makeup_pending')} / {t('attendance.makeup_done')}"
                        )
                        makeup_label = f"<div class='lesson-meta'>{h(label_prefix)}: {h(makeup_names)}{more_suffix}</div>"
                    meta_bits = [
        ...
        form_class_picker = render_picker_block(
            ...
            CURRENT_LANG,
            {
                ...
                "form_day_of_week": selected_form_day_query,
                "form_time_slot": selected_form_slot_query,
                "form_teacher_id": selected_form_teacher_query,
                "form_classroom": selected_form_room_query,
            },
        ...
        if lesson_mode:
            ...
            if target_schedule_id:
                ...
                for ms in makeup_students:
                    makeup_students_by_user_id[str(ms["user_id"])] = ms
                ...
                for st in students_in_class:
                    extra = makeup_students_by_user_id.get(str(st["user_id"]))
                    if extra and "source_lesson_date" not in st.keys():
                        merged = {k: st[k] for k in st.keys()}
                        merged.update({
                            "source_attendance_id": extra["source_attendance_id"],
                            "assignment_status": extra["assignment_status"],
                            "source_lesson_date": extra["source_lesson_date"],
                            "source_class_name": extra["source_class_name"],
                            "homeroom_teacher_name": extra["homeroom_teacher_name"],
                            "makeup_lesson_date": extra["makeup_lesson_date"],
                        })
                        enriched_students.append(merged)
                    else:
                        enriched_students.append(st)
                students_in_class = enriched_students
            ...
            if method == "POST" and has_role(user, [ROLE_OWNER, ROLE_MANAGER, ROLE_TEACHER]):
                ...
                for st in students_in_class:
                    ...
                    row = {
                        ...
                        "is_makeup_student": ("source_attendance_id" in st.keys()),
                    }
                ...
                if errs or row_errors:
                    ...
                    for row in parsed:
                        row_credit_delta = 0.0 if row["is_makeup_student"] else calc_credit_delta(row["status"], row["absence_charge_type"], class_credit_unit)
                        ...
                        if target_schedule_id is not None and row["is_makeup_student"] and row["status"] in ("present", "late"):
            ...
            for st in students_in_class:
                ...
                if 'source_lesson_date' in st.keys():
                    assignment_status = st["assignment_status"] if "assignment_status" in st.keys() else "assigned"
                    makeup_status_label = t('attendance.makeup_pending') if assignment_status == 'assigned' else t('attendance.makeup_done')
                    makeup_date_label = st["makeup_lesson_date"] if "makeup_lesson_date" in st.keys() else ""
                    extra_makeup_date = f" / {h(makeup_date_label)}" if makeup_date_label else ""
                    makeup_badge = f"<div class='muted'>{makeup_status_label}: {h(st['source_lesson_date'] or '-')} / {h(st['source_class_name'] or '-')} / {h(homeroom_display_name(st['homeroom_teacher_name']))}{extra_makeup_date}</div>"
                student_rows_html += f"""
                <tr>
                  <td>{h(st['student_no'] or '-')}</td>
    ...
    if path == "/library":
        ...
        <div class='card'><h4>{t("library.books")}</h4>{'' if load_library else ("<div class='empty-msg'>" + t('common.query_to_load') + "</div>")}<table><tr>{render_sort_th(t("field.id"), 'id', book_sort_by, book_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'loan_sort_by':loan_sort_by,'loan_sort_dir':loan_sort_dir,'loan_page':loan_page}, page_param='book_page')}{render_sort_th(t("field.code"), 'code', book_sort_by, book_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'loan_sort_by':loan_sort_by,'loan_sort_dir':loan_sort_dir,'loan_page':loan_page}, page_param='book_page')}{render_sort_th(t("field.title"), 'title', book_sort_by, book_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'loan_sort_by':loan_sort_by,'loan_sort_dir':loan_sort_dir,'loan_page':loan_page}, page_param='book_page')}{render_sort_th(t("field.status"), 'status', book_sort_by, book_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'loan_sort_by':loan_sort_by,'loan_sort_dir':loan_sort_dir,'loan_page':loan_page}, page_param='book_page')}</tr>{book_rows if load_library else ''}{(f"<tr><td colspan='4' class='empty-msg'>{t('common.no_data')}</td></tr>") if (load_library and not book_rows) else ''}</table>{render_pagination('/library', CURRENT_LANG, book_page, book_page_size, len(books), books_has_more, {**library_keep_params, 'loan_page': loan_page}, total_count=books_total_count, page_param='book_page') if load_library else ''}</div>
        <div class='card'><h4>{t("library.history")}</h4>{'' if load_library else ("<div class='empty-msg'>" + t('common.query_to_load') + "</div>")}<table><tr>{render_sort_th(t("field.code"), 'code', loan_sort_by, loan_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'book_sort_by':book_sort_by,'book_sort_dir':book_sort_dir,'book_page':book_page}, page_param='loan_page')}{render_sort_th(t("field.title"), 'title', loan_sort_by, loan_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'book_sort_by':book_sort_by,'book_sort_dir':book_sort_dir,'book_page':book_page}, page_param='loan_page')}{render_sort_th(t("field.name"), 'student_name', loan_sort_by, loan_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'book_sort_by':book_sort_by,'book_sort_dir':book_sort_dir,'book_page':book_page}, page_param='loan_page')}{render_sort_th(t("students.field.student_no"), 'student_no', loan_sort_by, loan_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'book_sort_by':book_sort_by,'book_sort_dir':book_sort_dir,'book_page':book_page}, page_param='loan_page')}{render_sort_th(t("field.loaned_at"), 'loaned_at', loan_sort_by, loan_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'book_sort_by':book_sort_by,'book_sort_dir':book_sort_dir,'book_page':book_page}, page_param='loan_page')}{render_sort_th(t("field.returned_at"), 'returned_at', loan_sort_by, loan_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'book_sort_by':book_sort_by,'book_sort_dir':book_sort_dir,'book_page':book_page}, page_param='loan_page')}{render_sort_th(t("field.handler"), 'handler_name', loan_sort_by, loan_sort_dir, '/library', CURRENT_LANG, {'load':'1','selected_student_id':selected_student_id,'book_sort_by':book_sort_by,'book_sort_dir':book_sort_dir,'book_page':book_page}, page_param='loan_page')}</tr>{loan_rows if load_library else ''}{(f"<tr><td colspan='7' class='empty-msg'>{t('common.no_data')}</td></tr>") if (load_library and not loan_rows) else ''}</table>{render_pagination('/library', CURRENT_LANG, loan_page, loan_page_size, len(loans), loans_has_more, {**library_keep_params, 'book_page': book_page}, total_count=loans_total_count, page_param='loan_page') if load_library else ''}</div>
